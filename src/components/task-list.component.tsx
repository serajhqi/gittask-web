...
export default function TaskList() {
  ...
  const [selectedTask] = useObservable(GetTaskRepo().selectedTask$)



  return <div className="flex flex-col border-2 border-black rounded-2xl bg-white overflow-auto h-fit no-scrollbar">
    {tasks?.map((task) =>
      <div key={task.id} className={`border-b-2 border-gray-500 border-dashed first:rounded-t-2xl hover:bg-sky-50 ${task.id == selectedTask?.id ? "bg-green-100" : ""}`}>
        <TaskItem task={task} onClick={() => GetTaskRepo().setSelected(task)} />
      </div>
    )}
